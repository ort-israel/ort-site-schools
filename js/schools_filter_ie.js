...
jQuery(document).ready(function ($) {
    ...
    function getCitiesinMap(cityFilecontents, mapBounds) {
        return cityFilecontents.filter(function (city) {
            // first check if the map contains this city's location
            var mapContainsCity = mapBounds.contains(city.location);
            ...
            if (!mapContainsCity) {
                ...
                var cityBounds = new google.maps.LatLngBounds(city.southWestViewport, city.northEastViewport); // check if the city bounds contain this map's center
    ...
    function applyCityClickToMap(result) {
        ...
        if (result.southWestViewport && result.northEastViewport) {
            var resultBounds = new google.maps.LatLngBounds(result.southWestViewport, result.northEastViewport);
    ...
    function updateJsonFile(cities) {
        var newData = JSON.stringify(cities);
        $.post(schools_and_map_filter_ajax_obj.ajax_url, {
            //POST request
    ...
    function changeMarkerIcon(event) {
        var eLatLng = event.latLng; //this will remove the previous marker

        removeProviouslySelectedMarker(); // create new marker with the location of the marker that was clicked
    ...
    function isElementInViewport(el) {
        //Use jQuery
        if (typeof $ === "function" && _instanceof(el, $)) {
            el = el[0];
        }
