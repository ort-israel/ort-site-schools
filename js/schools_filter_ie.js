/**
 * This file is adapted to Internet Explorer.
 * There are 2 isuues that had to be addressed:
 * 1. Arrow functions (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions#Browser_compatibility)
 * 2. Template literals (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#Browser_compatibility)
 */
            ...
            if (mapBounds !== null) {
                $.getJSON(schools_and_map_filter_ajax_obj.json_file + "?ver=" + Date.now(), function (data) {
                    ...
                    $('.elementor-accordion .elementor-tab-title a').each(function (key, value) {
                        ...
                        let cityExists = res.filter(function (item) {
    ...
    function updateCitiesFile() {
        $.getJSON(schools_and_map_filter_ajax_obj.json_file + "?ver=" + Date.now(), function (data) {
            ...
            $('.elementor-accordion .elementor-tab-title a').each(function (key, cityElement) {
                // if the innerHTML, which is the city name, doesn't exist in the list of filtered cities, hide it
                let cityExists = data.filter(function (item) {
